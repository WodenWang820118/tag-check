...
テストケースを監査する前に、参照用の report を作成します。同じイベントが複数箇所で発火することがあるため、システムは各 report に event ID を割り当てます。`add_to_cart` のテストケースが複数ある場合、全体で共有する JSON 仕様は 1 つで足ります。例:
...
Google Tag Manager は、Web サイトのコードを直接変更せずにタグを管理・配信できるタグ管理システムです。TagCheck は、繰り返しの手動テストを減らし、ビジネス要件に対してより高品質な QA を行えるよう支援することを目的としています。
