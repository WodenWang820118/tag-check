...
To deliver a standalone desktop experience, the application now runs inside a Tauri shell. The desktop runtime lives in `apps/desktop-tauri`, while `scripts/tauri/prepare-runtime.ts` prepares the Nest backend sidecar and bundled Node runtime used by the desktop build.
