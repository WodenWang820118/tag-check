...
- If a requested field is unavailable, return it with null rather than omitting it.
- Never return raw logs, raw diffs, credentials, tokens, headers, environment variables, or full command output.
- For FETCH_HEAVY, summarize only the failure cause, affected task, and proposed fix intent.
- For UPDATE_FIX, require action to be one of APPLY, REJECT, RERUN_ENVIRONMENT_STATE.
- If the command or required input is invalid, return:
