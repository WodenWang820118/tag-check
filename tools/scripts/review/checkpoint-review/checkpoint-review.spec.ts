...
test('getReviewExecutionPlan follows the repo checkpoint fallback rules', () => {
  ...
  assert.deepEqual(
    ...
    [
      execution('implementation', 'gemini', 'general', 'gemini-3.5-flash-high'),
...
test('parseChangedFilesFromContext normalizes Windows-style paths', () => {
  assert.deepEqual(
    parseChangedFilesFromContext(
      [
        'Changed files:',
        '- .\\tools\\scripts\\review\\checkpoint-review\\checkpoint-review.ts',
        ''
      ].join('\n')
    ),
    ['tools/scripts/review/checkpoint-review/checkpoint-review.ts']
  );
});
  ...
  assert.equal(
    inferAutoReviewRisk({
      ...
      context: [
        ...
        '',
        'Summary:',
        '- Tighten shared reviewer guidance and tool-native reviewer profile.'
      ].join('\n'),
      focus: 'general'
    }),
    'high'
...
test('inferAutoReviewRisk marks access-control and env config changes as high risk', () => {
  assert.equal(
    inferAutoReviewRisk({
      ...
      context: [
        'Changed files:',
        '- libs/utils/src/security/access-control.ts',
  ...
  assert.equal(
    inferAutoReviewRisk({
      ...
      context: [
        ...
        '- libs/utils/src/clients/UpstreamClient.java',
        '',
        'Summary:',
...
test('inferAutoReviewRisk marks permission policy changes as high risk', () => {
  assert.equal(
    inferAutoReviewRisk({
      ...
      context: [
        'Changed files:',
        '- libs/utils/src/security/PermissionPolicy.java',
        '',
        'Summary:',
...
test('buildReviewPrompt includes the checkpoint, focus, and supplied context', () => {
  ...
  assert.match(prompt, /No material issues found/);
  assert.match(
    prompt,
    /Changed files: tools\/scripts\/review-gate\/shared\/shared\.ts/
  );
...
test('executeReviewFlow reports all unavailable providers when auto routing is exhausted', async () => {
  await assert.rejects(
    ...
    /Attempted providers:[\s\S]*copilot:claude-sonnet-4\.6: copilot down[\s\S]*gemini:gemini-3\.5-flash-high: gemini down[\s\S]*copilot:gpt-5-mini: copilot down[\s\S]*codex: codex down/
  );
});
