...
export class StepExecutor {
  ...
  async executeStep(page: Page, step: any, projectName: string, testName) {
  ...
  async handleNavigate(page: Page, step: any) {
    ...
    if (this.getIsFirstNavigation()) {
      // only reload the landing page, trying to skip the overlay
      await page.reload({
        waitUntil: 'networkidle2',
      });
      this.setIsFirstNavigation(false);
    }
  }

  getIsFirstNavigation(): boolean {
    return this.isFirstNavigation;
  }

  setIsFirstNavigation(isFirstNavigation: boolean): void {
    this.isFirstNavigation = isFirstNavigation;
  }
}
