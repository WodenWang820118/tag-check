...
Prefer running project tasks through Nx-backed package scripts. Normal test
commands should stay non-coverage by default; coverage is opt-in through
`--coverage`, a project-specific coverage target, or the root Sonar path.

Backend test entrypoints:
...
`pnpm run test:e2e` is an alias for the non-coverage backend e2e path. The
backend e2e coverage target writes to `coverage/apps/nest-backend-e2e` so it
does not overwrite backend unit coverage.

Browser e2e entrypoint:

```bash
pnpm run test-front:e2e
```

Sonar coverage is generated by:
...
- Unit-style `test` targets: `ng-product-doc`, `ng-gtm-sample`,
  `shared-styles`, `data-access`, `ng-frontend`, `ui`.
- Backend split targets: `nest-backend:test-unit`,
  `nest-backend:test-integration`, `nest-backend:test-coverage`,
  `nest-backend:test-e2e`, `nest-backend:test-e2e-coverage`.
- Browser e2e targets/projects: `ng-frontend:e2e`, `ng-frontend-e2e`,
  `ng-gtm-sample-e2e`.
- No default unit coverage opt-in yet: `desktop-tauri`, `ng-tag-build`,
  `utils`.
